...
async function withNextFixture(source, run) {
  ...
  try {
    await mkdir(join(nextRoot, "dist", "server", "app-render"), {
      recursive: true,
    });
    await writeFile(
...
test("patches all Next.js PPR metadata resume call sites and is idempotent", async () => {
  await withNextFixture(
    vulnerableAppRenderFixture,
    async ({ appRenderPath, projectRoot }) => {
      ...
      const secondResult = await patchNextAppRender({
        projectRoot,
        logger: null,
      });
...
test("refuses to patch an unexpected number of vulnerable call sites", async () => {
  const unknownLayout = vulnerableAppRenderFixture.replace(
    vulnerableStatement,
    fixedStatement,
  );
  ...
  await withNextFixture(
    ...
    async ({ appRenderPath, projectRoot }) => {
      await assert.rejects(
        patchNextAppRender({ projectRoot, logger: null }),
        /Expected three vulnerable metadata-resume statements.*found 2/,
      );
...
test("refuses to patch when the framework insertion anchor changes", async () => {
  const unknownLayout = vulnerableAppRenderFixture.replace(
    "This is used by server actions & client-side navigations",
    "Framework internals changed",
  );
  ...
  await withNextFixture(
    ...
    async ({ appRenderPath, projectRoot }) => {
      await assert.rejects(
        patchNextAppRender({ projectRoot, logger: null }),
        /Could not find the safe helper insertion point/,
      );
      assert.equal(await readFile(appRenderPath, "utf8"), unknownLayout);
    },
  );
