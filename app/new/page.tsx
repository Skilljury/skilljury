...
async function NewPageResults({ searchParams }: { searchParams: SearchParams }) {
...
export default function NewPage({ searchParams }: ListingPageProps) {
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-6 py-10 lg:px-10 lg:py-14">
      <ListingShell
        description="Browse the most recently seen skills in the imported catalog, with a stable tiebreak on current popularity."
        title="New skills"
      />
      <Suspense fallback={<ListingResultsSkeleton />}>
        <NewPageResults searchParams={searchParams} />
      </Suspense>
    </div>
  );
}
