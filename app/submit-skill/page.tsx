...
export async function generateMetadata(): Promise<Metadata> {
  return buildPageMetadata({
    title: "Skill submissions temporarily unavailable | SkillJury",
    description:
      "SkillJury is currently serving a read-only recovery catalog. New skill submissions and moderation are temporarily unavailable while live provider access is restricted.",
    indexable: false,
    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-8 px-6 py-10 lg:px-10 lg:py-14">
      <section className="rounded-[2rem] border border-border bg-card/80 px-6 py-8 shadow-sm lg:px-10">
        ...
        <h1 className="mt-6 text-5xl font-semibold tracking-tight text-foreground sm:text-6xl">
          Skill submissions are temporarily unavailable.
        </h1>
        <p className="mt-5 max-w-3xl text-base leading-8 text-muted-foreground">
          SkillJury is currently serving a read-only recovery catalog while live provider access is restricted. New skill submissions, repository prefill, moderation, and publishing will return when live service is restored.
        </p>
